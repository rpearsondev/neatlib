...
pub fn code_view_ui(ui: &mut bevy_egui::egui::Ui, mut code: &str) {
    let language = "rs";
    let theme = CodeTheme::from_memory(ui.ctx());

    let mut layouter = |ui: &bevy_egui::egui::Ui, string: &str, _wrap_width: f32| {
    ...
    ui.add(
        bevy_egui::egui::TextEdit::multiline(&mut code)
            .font(bevy_egui::egui::TextStyle::Monospace) // for cursor height
            .code_editor()
            .desired_rows(1)
            .lock_focus(true)
...
pub fn highlight(ctx: &bevy_egui::egui::Context, theme: &CodeTheme, code: &str, language: &str) -> LayoutJob {
    impl bevy_egui::egui::util::cache::ComputerMut<(&CodeTheme, &str, &str), LayoutJob> for Highlighter {
        fn compute(&mut self, (theme, code, lang): (&CodeTheme, &str, &str)) -> LayoutJob {
            self.highlight(theme, code, lang)
        }
    }

    type HighlightCache = bevy_egui::egui::util::cache::FrameCache<LayoutJob, Highlighter>;
...
pub struct CodeTheme {
    ...
    #[cfg(not(feature = "syntect"))]
    formats: enum_map::EnumMap<TokenType, bevy_egui::egui::TextFormat>,
}
...
impl CodeTheme {
    pub fn from_style(style: &bevy_egui::egui::Style) -> Self {
    ...
    pub fn from_memory(ctx: &bevy_egui::egui::Context) -> Self {
        if ctx.style().visuals.dark_mode {
            ctx.data()
                .get_persisted(bevy_egui::egui::Id::new("dark"))
                .unwrap_or_else(CodeTheme::dark)
        } else {
            ctx.data()
                .get_persisted(bevy_egui::egui::Id::new("light"))
                .unwrap_or_else(CodeTheme::light)
        }
    }

    pub fn store_in_memory(self, ctx: &bevy_egui::egui::Context) {
        if self.dark_mode {
            ctx.data().insert_persisted(bevy_egui::egui::Id::new("dark"), self);
        } else {
            ctx.data().insert_persisted(bevy_egui::egui::Id::new("light"), self);
        }
    }
}
...
impl CodeTheme {
    pub fn dark() -> Self {
        let font_id = bevy_egui::egui::FontId::monospace(10.0);
    ...
    pub fn ui(&mut self, ui: &mut bevy_egui::egui::Ui) {
        ui.horizontal_top(|ui| {
            let selected_id = bevy_egui::egui::Id::null();
            ...
            ui.vertical(|ui| {
                ui.set_width(150.0);
                bevy_egui::egui::widgets::global_dark_light_mode_buttons(ui);
                ...
                if ui
                    .add_enabled(*self != reset_value, bevy_egui::egui::Button::new("Reset theme"))
                    .clicked()
                {
                    *self = reset_value;
                }
            ...
            bevy_egui::egui::Frame::group(ui.style())
                .inner_margin(bevy_egui::egui::Vec2::splat(2.0))
                .show(ui, |ui| {
                    // ui.group(|ui| {
                    ui.style_mut().override_text_style = Some(bevy_egui::egui::TextStyle::Small);
                    ui.spacing_mut().slider_width = 128.0; // Controls color picker size
                    bevy_egui::egui::widgets::color_picker::color_picker_color32(
                        ui,
                        &mut self.formats[selected_tt].color,
                        bevy_egui::egui::color_picker::Alpha::Opaque,
                    );
                });
        });
    }
